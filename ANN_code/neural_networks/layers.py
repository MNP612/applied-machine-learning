...
class FullyConnected(Layer):
    ...
    def _init_parameters(self, X_shape: Tuple[int]) -> None:
        ...
        W = np.ones([self.n_in,self.n_out]) # initialize weights using self.init_weights
        b = np.zeros(self.n_out) # initialize biases to zeros

        dW = np.zeros([self.n_in,self.n_out])
        db = np.zeros(self.n_out)

        ### END YOUR CODE ###

        self.parameters = OrderedDict({"W": W, "b": b})
        self.cache: OrderedDict = {'Z' : 0, 'X' : 0} # what do you need cache for backprop?
        self.gradients: OrderedDict = {"W": dW, "b": db}# initialize parameter gradients to zeros
    ...
    def forward(self, X: np.ndarray) -> np.ndarray:
        ...
        Z =  X @ W + b # some intermediate quantity
        out = self.activation.forward(Z) # activation

        # store information necessary for backprop in `self.cache`
        self.cache['Z'] = Z # something useful for backpropagation
        self.cache['X'] = X
    ...
    def backward(self, dLdY: np.ndarray) -> np.ndarray:
        ...
        # input of the layer
        dLdZ = self.activation.backward(Z, dLdY) # backward activation
